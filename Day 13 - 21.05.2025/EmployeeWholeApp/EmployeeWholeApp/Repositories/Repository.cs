...
using EmployeeWholeApp.Exception;
using EmployeeWholeApp.Interfaces;

namespace EmployeeWholeApp.Repositories
{
    public abstract class Repository<K, T> : IRepository<K, T> where T : class
    {
        ...
        public T Update(T item)
        {
            ...
            //var myItem = _items.FirstOrDefault(i => i.GetType().GetProperty("Id").GetValue(i).Equals(item.GetType().GetProperty("Id").GetValue(item)));
            if (myItem == null)
            {
                throw new KeyNotFoundException("Item not found");
            }
